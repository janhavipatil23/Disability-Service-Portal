 ...
 */
public class ManageMoneyAdvisorOrders extends javax.swing.JPanel {

    /**
     * Creates new form ManageMoneyAdvisorOrders
     */
    ...
    private MoneyWorkRequest selectedDir;
    DefaultTableModel model;
    DefaultTableModel ordersModel;

    public ManageMoneyAdvisorOrders(JPanel userProcessContainer, 
    ...
    public void showUpdateList() {
        ...
        for (int i = 0; i < moneyOrganization.getMoneyAdvisorsList().size(); i++) {
            assignCombo.addItem(moneyOrganization.getMoneyAdvisorsList().get(i).getUsername());
        }
    }

    public void viewOrderDirList() {
        for (int i = 0; i < moneyOrganization.getWorkQueue().getWorkRequestList().size(); i++) {
            MoneyWorkRequest advisor = (MoneyWorkRequest) moneyOrganization.getWorkQueue().getWorkRequestList().get(i);
            
            ordersModel.addRow(new Object[]{
                moneyOrganization.getWorkQueue().getWorkRequestList().get(i).getId(),
                advisor.getDate(),
                advisor.getTimeSlot(),
                advisor.getPrice(),
                moneyOrganization.getWorkQueue().getWorkRequestList().get(i).getStatus()
            });
        }
    }
    ...
    private void finishBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_finishBtnActionPerformed
        ...
        if (assignCombo.getSelectedItem() != null && selectedDir != null) {
            ...
            for (int j = 0; j < moneyOrganization.getMoneyAdvisorsList().size(); j++) {
                if (moneyOrganization.getMoneyAdvisorsList().get(j).getUsername().equals(selectedItem)) {
                    ...
                    viewOrderDirList();
                    JOptionPane.showMessageDialog(this, "YOUR SERVICE IS ACCEPTED AND MONEY ADVISOR IS ASSIGNED");
                    return;
                } else {
    ...
    private void rejectBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_rejectBtnActionPerformed
        ...
        if (selectedDir != null) {
            ...
            for (int i = 0; i < moneyOrganization.getWorkQueue().getWorkRequestList().size(); i++) {
                if (moneyOrganization.getWorkQueue().getWorkRequestList().get(i).getId() == (selectedDir.getId())) {
                    moneyOrganization.getWorkQueue().getWorkRequestList().remove(i);
    ...
    private void selectedButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_selectedButtonActionPerformed
        ...
        selectedDir = (MoneyWorkRequest) moneyOrganization.getWorkQueue().getWorkRequestList().get(orderListTable.getSelectedRow());
